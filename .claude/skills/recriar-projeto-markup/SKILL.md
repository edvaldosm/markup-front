...
description: Recria o projeto Markup do zero (backend Go + frontend Vue) a partir das bases de conhecimento .claude/backend-markup e .claude/frontend-markup, respeitando as Rules. Use quando o usuário pedir para recriar/scaffoldar/reconstruir o projeto inteiro.
---
...
- Backend: `.claude/backend-markup/README.md` → 7 Rules + 8 Skills
- Frontend: `.claude/frontend-markup/README.md` → 7 Rules + 8 Skills
...
1. **Confirmar destino** com o usuário: diretórios de saída (ex.: `markup-backend/`
   e `markup-front/` ou o repo atual) e se pode sobrescrever. Não apagar nada sem
   confirmação.
2. **Backend** — delegar à skill [[recriar-backend-markup]] (ou executar as fases
   dela): estrutura Go → schema GraphQL → domínio GORM → services (fórmula) →
   auth JWT → RBAC → seed.
...
4. **Integração** — garantir o contrato entre os dois:
   ...
   - Os tipos do front (`src/types`) batem com o schema GraphQL do back.
5. **Verificar** — backend: `go build ./...`; frontend: `npm install` + `npm run build`.
