...
7. **Router + layout** — `roteamento-e-layout`: guard (permissão **e** escopo),
   `AppLayout`/Sidebar/Header, `CompanySwitcher` e as 17 telas em `src/views/`.
8. **UI base** — componentes `Base*`, `StatCard`, `InfiniteScrollSentinel`, modais.
8b. **Gestão do Site** — `modulo-gestao-site`: rotas `/admin*` com
   `meta.adminGlobal`, `stores/admin.ts` de escopo global, as 4 telas do gestor e
   o escopo de tema `.theme-admin`.
